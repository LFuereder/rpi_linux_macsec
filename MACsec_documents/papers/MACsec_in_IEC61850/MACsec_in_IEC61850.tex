...
\noindent To test the mode of communication in a SAS, we implement a test setup consisting of three Bay Level components. As the IEDs take part in all 
forms of message exchange inside the Substation Architecture, they are perfectly suitable for testing the communication in conjunction with MACsec. In 
order to ensure the reproducibility of the study, we decided to use the Raspberry Pi 4 as hardware plattform. With respect to the software used in the 
applications, we utilize the open source library libiec61850\footnote{source: https://libiec61850.com/} to establish the different communication types 
and data structures of the IEC 61850 standard. To integrate MACsec into the communication, we utilize the MACsec Linux kernel module\footnote{source: 
https://github.com/torvalds/linux/blob/master/drivers/net/macsec.c}, which establishes a configurable virtual interface on top of an existing network 
interface \cite{MACsecLinuxModuleDoc:2016}. The remainder of this chapter initially explains the overall structure of the test setup and following this, 
further elaborates on the subsequent test executions. 

\subsection{Structure of the Test Setup}

\subsection{MACsec Implementation in the Test Setup}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.45\textwidth]{images/TestSetupIEDs.png}
    \caption{Structure of the Test Setup}
...
In our case, we integrated a measurement unit (MMXU) \cite[p. 268]{IEC61850-7-4:2010} and a control unit (LLN0) \cite[p. 164]{IEC61850-7-4:2010} into 
the SCL file. During runtime, IED1 continuously populates the data points of the measuring unit with sampled values of a sinusoidal function. In addition 
to the actual measurement, each sampled value contains a time stamp and a quality index \cite[p. 61ff]{IEC61850-7-3:2010}.
