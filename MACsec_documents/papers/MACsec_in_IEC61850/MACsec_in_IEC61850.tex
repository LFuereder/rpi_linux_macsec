...
periodically publish measurement information to all subscribing communication partner in the Bay Level via Sampled-Value (SV) packages 
...
(=Data Link Layer) of the OSI model \cite{TyphoonHIL_IEC61850SV:2021}. 
...
is then communicated through Manufacturing Message Specification (MMS) packages to other IEDs and the Station Level components \cite[p. 44]{IEC61850-8-1:2011}. 
Simultaniously the IEDs receive control signals from the Station Level, which are also transmitted via MMS packages \cite{trafficGen_IEC61850:2011}. 
As the Station Level components are not necessarily located in the same LAN as the IEDs, the MMS messaging is implmented through TCP packages on the 
...
Ethernet packages in the LAN \cite{GOOSE_confidentiality_integrity:2020}.

\smallskip
The devices located in the Station Level of the architecture are responsible for controlling the SAS. This is achieved through MMS packages adressed 
...
packets are extended by adding network and transport layer headers to form a UDP package, which is routable throughout a Wide Area Network (WAN) 
