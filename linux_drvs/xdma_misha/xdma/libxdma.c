...
#define xlx_wait_event_interruptible_timeout(wq, condition, timeout) \
	({                                                           \
		...
		do {                                                 \
			__ret = wait_event_interruptible_timeout(    \
...
static struct xdma_transfer *engine_start(struct xdma_engine *engine)
{
	...
	if (enable_st_c2h_credit && engine->streaming &&
	    engine->dir == DMA_FROM_DEVICE)
		write_register(engine->desc_used, &engine->sgdma_regs->credits,
			       0);
	...
	next_adj =
		xdma_get_next_adj(transfer->desc_adjacent,
			     ...
			     struct xdma_transfer *transfer, u32 desc_completed)
{
	...
	printk_ratelimited(KERN_INFO
			   ...
			   engine->name, engine->status, transfer,
...
static int engine_alloc_resource(struct xdma_engine *engine)
{
	...
	engine->desc = dma_alloc_coherent(
		&xdev->pdev->dev, engine->desc_max * sizeof(struct xdma_desc),
	...
	if (poll_mode) {
		engine->poll_mode_addr_virt = dma_alloc_coherent(
			&xdev->pdev->dev, sizeof(struct xdma_poll_wb),
			 ...
			 struct xdma_transfer *xfer)
{
	...
	for (i = 0; i < xfer->desc_num; i++) {
		u32 next_adj = xdma_get_next_adj(
			xfer->desc_num - i - 1, (xfer->desc_virt + i)->next_lo);
			   ...
			   bool dma_mapped, int timeout_ms)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 16, 0)
		sgt->nents =
			pci_map_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
#else
	...
	while (req->offset < req->total_len) {
		...
		for (sg = req->sg; i < sg_max && desc_idx < desc_max;
			...
			while (tlen) {
				unsigned int len =
					min_t(unsigned int, tlen, maxlen);
		...
		desc_virt->control =
			cpu_to_le32(DESC_MAGIC | XDMA_DESC_STOPPED |
...
int xdma_performance_submit(struct xdma_dev *xdev, struct xdma_engine *engine)
{
	...
	engine->perf_buf_virt =
		dma_alloc_coherent(&xdev->pdev->dev, size_in_desc,
