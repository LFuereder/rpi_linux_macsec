...
MODULE_DESCRIPTION("A simple DMA example for demonstrating MEM-to-MEM DMA");

/**
 * @brief Callback function for the completion of the the DMA Memcpy.  
 */
...
static int __init my_init(void)
{
	...
	src_buf = dma_alloc_coherent(chan->device->dev, 1024, &src_addr,
				     GFP_KERNEL);
	dst_buf = dma_alloc_coherent(chan->device->dev, 1024, &dst_addr,
				     GFP_KERNEL);
