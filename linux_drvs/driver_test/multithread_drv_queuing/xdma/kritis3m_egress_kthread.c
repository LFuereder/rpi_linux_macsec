...
ssize_t egress_thread_add_work(const char *buf, size_t count)
{
	...
	/* wake up egress thread, if it is currently sleeping */
...
static int __init init_egress_driver(void)
{
	...
	egress_driver =
		kzalloc(sizeof(struct kritis3m_egress_queue), GFP_KERNEL);
	if (!egress_driver)
		return -ENOMEM;

	/* initialize file pointer to associated character device */
	if (init_cdev_fileptr() != 0)
		goto FILP_ERROR;
	...
	egress_driver->egress_kthread = kthread_create(
		egress_thread_main, &thread_ID, "egress_kthread");
