...
static void async_io_handler(unsigned long cb_hndl, int err)
{
	...
	if (!err)
		numbytes = xdma_xfer_completion(
			(void *)cb, xdev, engine->channel, cb->write,
			cb->ep_addr, &cb->sgt, 0,
...
/*
 ******************************************************************************************
 *           Integration of the cross-kernel Interface for the xdma driver                *
 ******************************************************************************************
 */

/// @brief Modified clearing function to free up the mapped scatterlist in the 
/// 	   kernel-space memory.
...
/// @brief Modified mapping function to map kernel-space memory to the scatterlist
///		   for a subsequent return of a control block containing the scatterlist. 
static int char_sgdma_map_kernel_buf_to_sgl(struct xdma_io_cb *cb, bool write)
{	
	struct sg_table *sgt = &cb->sgt;
	unsigned long len = cb->len;
	void *buf = cb->buf; // Kernel-space buffer
	struct scatterlist *sg;
	unsigned int pages_nr = (((unsigned long)buf + len + PAGE_SIZE - 1) -
				 			((unsigned long)buf & PAGE_MASK)) >> PAGE_SHIFT;
	int i;

	if (pages_nr == 0) return -EINVAL;

	// Allocate scatterlist for the number of pages
	if (sg_alloc_table(sgt, pages_nr, GFP_KERNEL)) {
		pr_err("sgl OOM.\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < pages_nr; i++, sg = sg_next(sg)) 
	{
		// Calculate page offset and number of bytes in this page
		unsigned int offset = offset_in_page(buf);
		unsigned int nbytes = min_t(unsigned int, PAGE_SIZE - offset, len);

		// Get the page struct for the kernel virtual address
		struct page *page = virt_to_page(buf); // Convert kernel virtual address to struct page

		// Add this page to the scatterlist
		sg_set_page(sg, page, nbytes, offset);

		// Move buffer pointer forward and reduce remaining length
		buf += nbytes;
		len -= nbytes;
	}
					   ...
					   loff_t *pos, bool write)
{
	...
	rv = char_sgdma_map_kernel_buf_to_sgl(&cb, write);
	if (rv < 0) return rv;
...
/// @param count Number of bytes in the buffer, which shall be transmitted via PCIe.
...
{
	struct xdma_cdev *xcdev =
		(struct xdma_cdev *)iocb->ki_filp->private_data;
...
static ssize_t cdev_write_iter(struct kiocb *iocb, struct iov_iter *io)
{
	...
	return cdev_aio_write(
		iocb, iter_iov(io), io->nr_segs,
		io->iov_offset); // 2024011901: linux-6.4.0  - include/linux/uio.h line 72:73
#else
	return cdev_aio_write(
		iocb, io->iov, io->nr_segs,
		io->iov_offset); // 2024011901: linux-6.3.13 - include/linux/uio.h line 54
#endif
}
