...
#define for_each_rxsc(secy, sc)                        \
	for (sc = rcu_dereference_bh(secy->rx_sc); sc; \
	     sc = rcu_dereference_bh(sc->next))
#define for_each_rxsc_rtnl(secy, sc)                 \
	for (sc = rtnl_dereference(secy->rx_sc); sc; \
...
static bool macsec_validate_skb(struct sk_buff *skb, u16 icv_len, bool xpn)
{
	...
	int extra_len =
		macsec_extra_len(!!(h->tci_an & MACSEC_TCI_SC)) + icv_len;
...
{
	printk("[ INFO ] now in MACsec Encrypt function ");
	...
	if (unlikely(skb_headroom(skb) < MACSEC_NEEDED_HEADROOM ||
		...
		struct sk_buff *nskb =
			skb_copy_expand(skb, MACSEC_NEEDED_HEADROOM,
					MACSEC_NEEDED_TAILROOM, GFP_ATOMIC);
		if (likely(nskb)) {
			consume_skb(skb);
			skb = nskb;
		} else {
	...
	else if(HW_AEAD_AVAILABLE && tx_sc->encrypt)
	{
		ret = macsec_encrypt_on_HW(skb);
	}
...
static struct sk_buff *macsec_decrypt(struct sk_buff *skb,
				      struct net_device *dev,
				      struct macsec_rx_sa *rx_sa, sci_t sci,
				      struct macsec_secy *secy)
{
	int ret;
	struct scatterlist *sg;
	...
	if (hdr->tci_an & MACSEC_TCI_E) {
		 ...
		 */
		int len =
			skb->len - macsec_hdr_len(macsec_skb_cb(skb)->has_sci);

		aead_request_set_crypt(req, sg, sg, len, iv);
		aead_request_set_ad(
			req, macsec_hdr_len(macsec_skb_cb(skb)->has_sci));
		skb = skb_unshare(skb, GFP_ATOMIC);
		if (!skb) {
			aead_request_free(req);
			return ERR_PTR(-ENOMEM);
	...
	*pskb = skb;
	...
	if (!rx_sa) {
		...
		if (hdr->tci_an & MACSEC_TCI_C ||
			...
			if (active_rx_sa)
				this_cpu_inc(
					active_rx_sa->stats->InPktsNotUsingSA);
			goto drop_nosa;
		}
	...
	if (secy->replay_protect) {
		...
		late = rx_sa->next_pn_halves.lower >= secy->replay_window &&
		       hdr_pn < (rx_sa->next_pn_halves.lower -
				 secy->replay_window);

		if (secy->xpn)
			late = late && pn_same_half(rx_sa->next_pn_halves.lower,
						    hdr_pn);
		spin_unlock(&rx_sa->lock);
...
static struct macsec_rx_sc *del_rx_sc(struct macsec_secy *secy, sci_t sci)
{
	...
	for (rx_scp = &secy->rx_sc, rx_sc = rtnl_dereference(*rx_scp); rx_sc;
...
static struct macsec_tx_sa *
get_txsa_from_nl(struct net *net, struct nlattr **attrs, struct nlattr **tb_sa,
		 struct net_device **devp, struct macsec_secy **secyp,
		 struct macsec_tx_sc **scp, u8 *assoc_num)
{
	struct net_device *dev;
	struct macsec_secy *secy;
	struct macsec_tx_sc *tx_sc;
...
static struct macsec_rx_sa *
get_rxsa_from_nl(struct net *net, struct nlattr **attrs,
		 struct nlattr **tb_rxsc, struct nlattr **tb_sa,
		 struct net_device **devp, struct macsec_secy **secyp,
		 struct macsec_rx_sc **scp, u8 *assoc_num)
{
	struct macsec_rx_sc *rx_sc;
	struct macsec_rx_sa *rx_sa;
...
static const struct nla_policy
	macsec_genl_offload_policy[NUM_MACSEC_OFFLOAD_ATTR] = {
...
{
	if (!attrs[MACSEC_SA_ATTR_AN] || !attrs[MACSEC_SA_ATTR_KEY] ||
...
static int macsec_add_rxsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (secy->xpn) {
		if (!tb_sa[MACSEC_SA_ATTR_SSCI] ||
		    !tb_sa[MACSEC_SA_ATTR_SALT]) {
			rtnl_unlock();
			return -EINVAL;
		}
...
{
	if (!attrs[MACSEC_SA_ATTR_AN] || !attrs[MACSEC_SA_ATTR_PN] ||
	    !attrs[MACSEC_SA_ATTR_KEY] || !attrs[MACSEC_SA_ATTR_KEYID])
		return false;
...
static int macsec_del_txsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	rtnl_lock();
	tx_sa = get_txsa_from_nl(genl_info_net(info), attrs, tb_sa, &dev, &secy,
				 &tx_sc, &assoc_num);
	if (IS_ERR(tx_sa)) {
		rtnl_unlock();
...
{
	if (!attrs[MACSEC_SA_ATTR_AN] || attrs[MACSEC_SA_ATTR_KEY] ||
	    attrs[MACSEC_SA_ATTR_KEYID] || attrs[MACSEC_SA_ATTR_SSCI] ||
...
static int macsec_upd_rxsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (tb_sa[MACSEC_SA_ATTR_PN]) {
		...
		if (nla_len(tb_sa[MACSEC_SA_ATTR_PN]) != pn_len) {
			pr_notice(
				"macsec: nl: upd_rxsa: bad pn length: %d != %d\n",
				nla_len(tb_sa[MACSEC_SA_ATTR_PN]), pn_len);
			rtnl_unlock();
			return -EINVAL;
		}
...
static void get_rx_sa_stats(struct net_device *dev, struct macsec_rx_sc *rx_sc,
			    int an, struct macsec_rx_sa *rx_sa,
...
static int copy_rx_sa_stats(struct sk_buff *skb, struct macsec_rx_sa_stats *sum)
{
...
static void get_rx_sc_stats(struct net_device *dev, struct macsec_rx_sc *rx_sc,
...
static int copy_rx_sc_stats(struct sk_buff *skb, struct macsec_rx_sc_stats *sum)
{
	if (nla_put_u64_64bit(skb, MACSEC_RXSC_STATS_ATTR_IN_OCTETS_VALIDATED,
	    ...
	    nla_put_u64_64bit(skb, MACSEC_RXSC_STATS_ATTR_IN_PKTS_OK,
			      sum->InPktsOK, MACSEC_RXSC_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_RXSC_STATS_ATTR_IN_PKTS_INVALID,
			      sum->InPktsInvalid, MACSEC_RXSC_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_RXSC_STATS_ATTR_IN_PKTS_LATE,
			      sum->InPktsLate, MACSEC_RXSC_STATS_ATTR_PAD) ||
	    ...
	    nla_put_u64_64bit(skb, MACSEC_RXSC_STATS_ATTR_IN_PKTS_UNUSED_SA,
			      sum->InPktsUnusedSA, MACSEC_RXSC_STATS_ATTR_PAD))
		return -EMSGSIZE;
...
static int copy_secy_stats(struct sk_buff *skb, struct macsec_dev_stats *sum)
{
	if (nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_OUT_PKTS_UNTAGGED,
	    ...
	    nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_IN_PKTS_NO_TAG,
			      sum->InPktsNoTag, MACSEC_SECY_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_IN_PKTS_BAD_TAG,
			      sum->InPktsBadTag, MACSEC_SECY_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_IN_PKTS_UNKNOWN_SCI,
			      sum->InPktsUnknownSCI,
			      MACSEC_SECY_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_IN_PKTS_NO_SCI,
			      sum->InPktsNoSCI, MACSEC_SECY_STATS_ATTR_PAD) ||
	    nla_put_u64_64bit(skb, MACSEC_SECY_STATS_ATTR_IN_PKTS_OVERRUN,
			      sum->InPktsOverrun, MACSEC_SECY_STATS_ATTR_PAD))
		return -EMSGSIZE;
...
{
	struct macsec_tx_sc *tx_sc = &secy->tx_sc;
	struct nlattr *secy_nest = nla_nest_start_noflag(skb, MACSEC_ATTR_SECY);
	u64 csid;
	...
	if (nla_put_sci(skb, MACSEC_SECY_ATTR_SCI, secy->sci,
	    ...
	    nla_put_u64_64bit(skb, MACSEC_SECY_ATTR_CIPHER_SUITE, csid,
			      MACSEC_SECY_ATTR_PAD) ||
	...
	if (secy->replay_protect) {
		if (nla_put_u32(skb, MACSEC_SECY_ATTR_WINDOW,
				secy->replay_window))
			goto cancel;
	}
					...
					struct netlink_callback *cb)
{
	...
	for (i = 0, j = 1; i < MACSEC_NUM_AN; i++) {
		...
		if (nla_put_u8(skb, MACSEC_SA_ATTR_AN, i) ||
		    ...
		    nla_put(skb, MACSEC_SA_ATTR_KEYID, MACSEC_KEYID_LEN,
			    tx_sa->key.id) ||
		    (secy->xpn &&
		     nla_put_ssci(skb, MACSEC_SA_ATTR_SSCI, tx_sa->ssci)) ||
	...
	for_each_rxsc_rtnl(secy, rx_sc)
	{
		...
		rxsa_list =
			nla_nest_start_noflag(skb, MACSEC_RXSC_ATTR_SA_LIST);
		if (!rxsa_list) {
			nla_nest_cancel(skb, rxsc_nest);
			nla_nest_cancel(skb, rxsc_list);
			goto nla_put_failure;
...
#define MACSEC_FEATURES (NETIF_F_SG | NETIF_F_HIGHDMA | NETIF_F_FRAGLIST)
...
static int macsec_dev_init(struct net_device *dev)
{
	...
	dev->needed_headroom =
		real_dev->needed_headroom + MACSEC_NEEDED_HEADROOM;
	dev->needed_tailroom =
		real_dev->needed_tailroom + MACSEC_NEEDED_TAILROOM;
				    ...
				    struct nlattr *data[])
{
	...
	if (data[IFLA_MACSEC_VALIDATION])
		secy->validate_frames =
			nla_get_u8(data[IFLA_MACSEC_VALIDATION]);
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	if (data[IFLA_MACSEC_CIPHER_SUITE] || data[IFLA_MACSEC_ICV_LEN] ||
	    data[IFLA_MACSEC_SCI] || data[IFLA_MACSEC_PORT])
		return -EINVAL;
...
static void macsec_del_dev(struct macsec_dev *macsec)
{
	...
	while (macsec->secy.rx_sc) {
		struct macsec_rx_sc *rx_sc =
			rtnl_dereference(macsec->secy.rx_sc);
	...
	for (i = 0; i < MACSEC_NUM_AN; i++) {
		struct macsec_tx_sa *sa =
			rtnl_dereference(macsec->secy.tx_sc.sa[i]);
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	lockdep_set_class(&dev->addr_list_lock, &macsec_netdev_addr_lock_key);
				...
				struct netlink_ext_ack *extack)
{
	...
	if (data[IFLA_MACSEC_ICV_LEN]) {
		...
		if (icv_len != MACSEC_DEFAULT_ICV_LEN) {
			...
			dummy_tfm = macsec_alloc_tfm(dummy_key, DEFAULT_SAK_LEN,
						     icv_len);
			if (IS_ERR(dummy_tfm))
	...
	for (flag = IFLA_MACSEC_ENCODING_SA + 1; flag < IFLA_MACSEC_VALIDATION;
	     flag++) {
		if (data[flag]) {
...
static int macsec_fill_info(struct sk_buff *skb, const struct net_device *dev)
{
	struct macsec_tx_sc *tx_sc;
	struct macsec_dev *macsec;
	struct macsec_secy *secy;
	...
	if (nla_put_sci(skb, IFLA_MACSEC_SCI, secy->sci, IFLA_MACSEC_PAD) ||
	    nla_put_u8(skb, IFLA_MACSEC_ICV_LEN, secy->icv_len) ||
	    nla_put_u64_64bit(skb, IFLA_MACSEC_CIPHER_SUITE, csid,
			      IFLA_MACSEC_PAD) ||
	    ...
	    nla_put_u8(skb, IFLA_MACSEC_OFFLOAD, macsec->offload) || 0)
		goto nla_put_failure;
			 ...
			 void *ptr)
{
	...
	case NETDEV_CHANGEMTU: {
		...
		list_for_each_entry(m, &rxd->secys, secys) {
			...
			unsigned int mtu =
				real_dev->mtu -
				(m->secy.icv_len + macsec_extra_len(true));
...
MODULE_DESCRIPTION("LAS3 -- KRITIS3M MACsec IEEE 802.1AE");
